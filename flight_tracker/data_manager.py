...
class DataManager:
    ...
    def __init__(self, flight_search: FlightSearch) -> None:
        ...
        self.endpoint = (
            "https://api.sheety.co/2b8b564dbc8b5e35348b478b9f9509ff/flightDeals"
        )
    ...
    def get_destinations(self):
        ...
        data = data["prices"]
        data_list = [
            {"iataCode": destination["iataCode"], "price": destination["price"]}
    ...
    def add_destination(self, city_name: str, price: int):
        ...
        query = {"price": {"city": city_name, "iataCode": iata_code, "price": price}}

        response = requests.post(
            prices_endpoint, auth=(self.user, self.password), json=query
        )
        response.raise_for_status()
    ...
    def collect_user_data(self):
        ...
        if email:
            self.add_user(
                first_name=first_name, last_name=last_name, email=email, origin=origin
            )
    ...
    def add_user(self, first_name: str, last_name: str, email: str, origin: str):
        ...
        response = requests.post(
            users_endpoint, auth=(self.user, self.password), json=query
        )
        response.raise_for_status()

        if 200 <= response.status_code < 300:
            print(
                f"{first_name} {last_name} have been added successfully in the mailing list."
            )
