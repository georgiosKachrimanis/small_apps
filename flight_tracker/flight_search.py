...
class FlightSearch:
    ...
    def check_available_flights(self, origin_city_code:str, destination_city_code:str, date_from:datetime, date_to:datetime):
        ...
        try:
            response = requests.get(url=search_url, headers=self.headers, params=parameters)
            response.raise_for_status()  # Check for HTTPError
            data = response.json()
            if 'data' in data and len(data['data']) > 0:
                return data
            elif 'data' in data and len(data['data']) ==  0 and parameters['max_stopovers'] == 0:
                ...
                if 'data' in data and len(data['data']) > 0:
                    ...
                    print(f"No flights found for {destination_city_code} with 0 or 1 stopover. At the price you have set!")
                    return None
            else:
                print(f"No flights found for {destination_city_code} with 0 or 1 stopover. At the price you have set!")
                return None
