...
card_image = card.create_image(
    400, 263, image=front_img
)  # We will manipulate this image depending if we have the question or the answer.
# Image Text (Title)
card_title = card.create_text(
    400, 150, text="", fill=FILL_COLOR, font=("Ariel", 40, "italic")
)
# Image Text (Word)
card_word = card.create_text(
    400, 263, text="", fill=FILL_COLOR, font=("Ariel", 60, "bold")
)
...
not_known_word = Button(
    image=not_known_word_img, highlightthickness=0, borderwidth=0, command=next_card
)
not_known_word.grid(row=1, column=0, padx=10, pady=10)

known_word_img = PhotoImage(file="wordcard/images/right.png")
known_word = Button(
    image=known_word_img, highlightthickness=0, borderwidth=0, command=is_known
)
