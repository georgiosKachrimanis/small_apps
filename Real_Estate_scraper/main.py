...
GOOGLE_FORM_LINK = (
    ""  # YOU SHOULD UPDATE WITH THE LINK OF THE GOOGLE FORM YOU HAVE CREATED!
)
...
class WebsiteInteractionManager:
    ...
    def fill_search_parameters(self):
        ...
        try:
            ...
            house_type = self.driver.find_element(
                By.XPATH, value="/html/body/main/div[1]/div[3]/form/nav/ul/li[2]/a"
            )
            house_type.click()
            time.sleep(5)
            # Search Area
            area_box = self.driver.find_element(
                By.XPATH,
                value="/html/body/main/div[1]/div[3]/form/div/div[1]/fieldset[1]/div[1]/input[3]",
            )
            area_box.send_keys(AREA)
            time.sleep(5)

            # Search area range
            search_range_dropdown = Select(
                self.driver.find_element(By.ID, value="Straal")
            )
            search_range_dropdown.select_by_value(SEARCH_RANGE)
            time.sleep(5)

            # Maximum price
            max_price_dropdown = Select(
                self.driver.find_element(
                    By.ID, value="range-filter-selector-select-filter_huurprijstot"
                )
            )
            ...
            submit_search = self.driver.find_element(
                By.XPATH,
                value="/html/body/main/div[1]/div[3]/form/div/div[1]/div/button",
            )
            submit_search.click()
    ...
    def next_page(self):
        ...
        try:
            next_page = self.driver.find_elements(
                By.CSS_SELECTOR, value=".pagination li a"
            )
...
class RealEstateWebsiteScraper:
    ...
    def find_href_elements(self) -> list:
        try:
            href_elements = self.soup.find_all(
                "a", attrs={"data-test-id": "object-image-link"}
            )  # The picture link and the name link are the same...for now
    ...
    def find_text_elements(self, search_attributes: dict) -> list:
        try:
            ...
            cleaned_list = self.remove_garbage_chars(
                elements_list
            )  # Removes the extra characters and the white space
            return cleaned_list
        except Exception as e:
    ...
    def create_elements_list(self) -> list:
        ...
        streets = self.find_text_elements(
            search_attributes={"data-test-id": "street-name-house-number"}
        )

        # Get the postal Code
        postal_codes = self.find_text_elements(
            search_attributes={"data-test-id": "postal-code-city"}
        )

        # Get prices
        prices = self.find_text_elements(
            search_attributes={"data-test-id": "price-rent"}
        )
        prices_cleaned = self.numbers_only(
            list=prices
        )  # In order to keep only the numerical value
...
def main():
    ...
    for listing in range(len(results["Address"])):
        ...
        google_form_entry.submit_listing_to_form(
            address=address, price=price, link=link
        )
        time.sleep(5)

    print(
        "Scraping the pages is done\n You can now find all the data in the form or excel file. Terminating program in 10 secs!"
    )
